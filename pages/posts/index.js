    ...
    <>
      ...
      <div className="max-w-screen-sm px-4 mt-12 mb-24 mx-auto">     
        ...
        <section className="text-xl leading-6 p-1">
          <h1 className="text-3xl text-gray-600 text-opacity-50 dark:text-gray-400 leading-8 py-8">Posts</h1>
          ...
          <ul className="list-none p-0 m-0">
            {allPostsData.map(({ id, image, alt, title, date, description }) => (
              <li className="mb-6 flex" key={id}>
                ...
                <Link href={`/posts/${id}`}><Image src={image} alt={alt} width={100} height={100} className="rounded-lg" /></Link>        
              </li>
            ))}
          </ul>
        </section>
